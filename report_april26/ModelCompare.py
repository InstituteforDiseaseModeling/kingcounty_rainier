...
if __name__ == "__main__":
    ...
    axes.plot(forecast.index, forecast["high"], "#FAAF08", lw=3, ls="dashed",
              label="Mobility-based nowcast from the 4/10 report")

    ## Plot the data
    shift = pd.to_timedelta(0.1, unit="d")
    axes.errorbar(kc2_points.index - shift, kc2_points["r0_t"].values, yerr=2. * kc2_points["std_err"],
                  color="0.8", ls="None", marker="o",
                  label=r"R$_e$ estimates based on WDRS compiled on 4/3 (4/10 report)")
    axes.errorbar(kc3_points.index, kc3_points["r0_t"].values, yerr=2. * kc3_points["std_err"],
                  color="0.5", ls="None", marker="o",
                  label=r"R$_e$ estimates based on WDRS compiled on 4/19 (4/21 report)")
    axes.errorbar(kc4_points.index + shift, kc4_points["r0_t"].values, yerr=2. * kc4_points["std_err"],
                  color="k", ls="None", marker="o",
                  label=r"R$_e$ estimates based on WDRS compiled on 4/26 (this report)")
