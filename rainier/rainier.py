...
def continuous_time_posterior(p, model, cases, tr_start, debug=False):
    ...
    if debug:
        ...
        axes[1].fill_between(model.time[:-1], delta_E - 2. * delta_E_std, delta_E + 2. * delta_E_std, color="k",
                             alpha=0.25)
        ...
        axes[2].plot(model.time[1:], Shat, color="#375E97",
                     label="Estimated susceptible population\nbased on daily new exposures")
        axes[2].axvline(tr_start, c="k")
        axes[2].axvline(len(model.time) - model.D_e - 1, c="k")

        ## Step 4: log transmission rate panel
        Y_std = np.sqrt(Yvar)
        axes[3].fill_between(model.time[tr_start:-model.D_e - 1], Yhat - 2. * Y_std, Yhat + 2. * Y_std, color="#3F681C",
                             alpha=0.25)
...
def continuous_time_posterior_sampler(p, model, cases, num_spline_samples, tr_start, debug=False):
    ...
    if debug:
        ...
        axes[2].plot(model.time[1:], m, color="#375E97",
                     label="Estimated susceptible population\nbased on daily new exposures")
        axes[2].axvline(tr_start, c="k")
        axes[2].axvline(len(model.time) - model.D_e - 1, c="k")
